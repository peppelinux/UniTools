...
server = None
if args.usessl:
	server = smtplib.SMTP_SSL(host=serveraddr, port=serverport)
else:
	server = smtplib.SMTP(host=serveraddr, port=serverport)

server.set_debuglevel(args.debug)
server.connect(host=serveraddr, port=serverport)
server.ehlo()
